...
var _Sources = (() => {
  ...
  // src/MangaForFree/MangaForFree.ts
  var MangaForFree_exports = {};
  __export(MangaForFree_exports, {
    MangaForFree: () => MangaForFree,
    MangaForFreeInfo: () => MangaForFreeInfo
  });
  ...
  var Parser3 = class {
    ...
    async parseMangaDetails($2, mangaId, source) {
      ...
      const author = decode($2("div.author-content").first().text().replace("\\n", "").trim()).replace("Updating", "");
      const artist = decode($2("div.artist-content").first().text().replace("\\n", "").trim()).replace("Updating", "");
      const description = decode($2("div.description-summary, div.summary-container, div.manga-excerpt").first().text()).replace("Show more", "").trim();
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  // src/MangaForFree/MangaForFreeParser.ts
  var MangaForFreeParser = class extends Parser3 {
    constructor() {
      super(...arguments);
      this.parseDate = (date) => {
        date = date.toUpperCase();
  ...
  // src/MangaForFree/MangaForFree.ts
  var DOMAIN = "https://mangaforfree.com/";
  var MangaForFreeInfo = {
  ...
  var MangaForFree = class extends Madara {
    constructor() {
      super(...arguments);
      this.baseUrl = DOMAIN;
      this.chapterEndpoint = 1;
      this.parser = new MangaForFreeParser();
    }
  };
  return __toCommonJS(MangaForFree_exports);
})();
